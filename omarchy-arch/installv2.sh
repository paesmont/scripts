...
if [[ ! -d "$OMARCHY_PATH/.git" ]]; then
  ...
  git clone https://github.com/basecamp/omarchy "$OMARCHY_PATH"
else
...
sudo pacman -Sy --noconfirm omarchy/omarchy-keyring || true
...
backup_path="$OMARCHY_PATH/.backup-disabled"
mkdir -p "$backup_path"

for file in "${OMARCHY_SAFE_DISABLE[@]}"; do
  if [[ -f "$OMARCHY_PATH/$file" ]]; then
    mv "$OMARCHY_PATH/$file" "$backup_path/"
